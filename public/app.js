...
var poped = false; //global settings
...
$("document").ready(function () {
  axios.get("/memes").then(function (res) {
    renderMeme(res.data);
  });
...
function renderTextBox(textInfo) {
  ...
  for (var i = 0; i < textInfo.length; i++) {
    ...
    out = out.concat("\n        <div class=\"input-group mb-3\">\n            <input class=\"form-control ml-1 mr-1\" type=\"text\" id=\"textBox".concat(i, "\" placeholder=\"Text\"/>\n        </div>\n           "));
  }
...
function renderMeme(res) {
  ...
  res.map(function (item) {
    str = "[";
    item.textInfo.map(function (element) {
...
function canvasFunc(img, textInfo) {
  ...
  currentImage.src = "/images/".concat(img);
  var ratio = 1;
  ...
  function Square(x, y) {
    ...
    this.render = function () {
      ...
      ctx.fillRect(_this.x, _this.y, 20, 20);
      ctx.fillStyle = "rgba(0, 0, 0, 1)";
      ctx.fillRect(_this.x + 2.5, _this.y + 2.5, 15, 15);
    };
  }
  ...
  function TextBox(x, y, width, height, input) {
    ...
    this.style = "#000000";
    this.squares = [new Square(x, y), new Square(x, y + height), new Square(x + width, y + height), new Square(x + width, y)];
    this.clicked = false;
    this.offsetX = 0;
    this.offsetY = 0;
    this.doRender = false;
    ...
    this.render = function () {
      ...
      this.squares.map(function (item) {
        return item.render();
      });
    ...
    this.update = function () {
      ...
      if (mouse.x > this.x + 7 && mouse.x < this.x + this.width + 7 && mouse.y > this.y + 7 && mouse.y < this.y + this.height + 7 && !this.squares.some(function (item) {
        return item.clicked;
      })) {
      ...
      ctx.font = "".concat(fontWidth, "px ").concat(font);
      ctx.fillStyle = fontColor;
      ctx.strokeStyle = fontOutline; // ctx.strokeText(`${x} ${y}  ${width} ${height}`, x + width/2, y + height/2);
  ...
  function loadAllTheBoxes() {
    ...
    var lastIndex = textInfo.length - 1;
    out += "<div class=\"input-group mb-3\">\n                    <input class=\"form-control ml-1 mr-1\" type=\"text\" id=\"textBox".concat(lastIndex, "\" placeholder=\"Text\"/>\n                </div>");
    container.innerHTML = out;
    textBoxes.push(new TextBox(textInfo[lastIndex].x, textInfo[lastIndex].y, textInfo[lastIndex].width, textInfo[lastIndex].height, "textBox".concat(lastIndex))); //renderTextBox(textInfo);
  }

  for (var i = 0; i < textInfo.length; i++) {
    textBoxes.push(new TextBox(textInfo[i].x, textInfo[i].y, textInfo[i].width, textInfo[i].height, "textBox".concat(i)));
  }

  renderTextBox(textInfo);
  document.getElementById("canvas").addEventListener("mouseover", function () {
    textBoxes.map(function (item) {
      item.doRender = true;
    });
  });
...
function renderSettings() {
  ...
  $(document).on("input", ".outline-input", function () {
    $(".outline-badge").css("background", $(".font-outline input").val());
  });
  $(document).on("input", ".font-size-input input", function () {
    fontWidth = $(".font-size-input input").val();
  });
  $(document).on("input", ".font-input input", function () {
    font = $(".font-input input").val();
  });
  $(document).on("input", ".font-color input", function () {
    fontColor = $(".font-color input").val();
  });
  $(document).on("input", ".font-outline input", function () {
    fontOutline = $(".font-outline input").val();
  });
}
